...
def noise_estimation_loss(model, x0, t, e, b,input_channel=3):
    a = (1-b).cumprod(dim=0).index_select(0, t).view(-1, 1, 1, 1)
    x = x0[:, input_channel:, :, :] * a.sqrt() + e * (1.0 - a).sqrt()
    # print(x.shape,x0.shape)
    # print(torch.cat([x0[:, :1, :, :], x], dim=1).shape)
    output = model(torch.cat([x0[:, :input_channel, :, :], x], dim=1), t.float())
    # output = model(x0[:, :1, :, :], t.float())
...
class DenoisingDiffusion(object):
    def __init__(self, args, config):
        ...
        self.input_chn_num=config.model.in_channels
        self.output_chn_num=config.model.out_ch
    ...
    def train(self, DATASET):
        ...
        for epoch in range(self.start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                x = x.to(self.device)
                x = data_transform(x)
                e = torch.randn_like(x[:, self.input_chn_num:, :, :])
                b = self.betas
                ...
                loss = noise_estimation_loss(self.model, x, t, e, b,input_channel=self.input_chn_num)
                ...
                if self.step % self.config.training.snapshot_freq == 0 or self.step == 1:
                    utils.logging.save_checkpoint({
                        ...
                        'params': self.args,
                        'config': self.config
                    }, filename=os.path.join(self.config.data.save_model_dir, 'ckpts', self.config.data.model_name + '_ddm'))
    ...
    def sample_validation_patches(self, val_loader, step):
        ...
        with torch.no_grad():
            ...
            for i, (x, y) in enumerate(val_loader):
                ...
                break
            n = x.size(0)
            x_cond = x[:, :self.input_chn_num, :, :].to(self.device)
            x_cond = data_transform(x_cond)
            x = torch.randn(n, self.output_chn_num, self.config.data.image_size, self.config.data.image_size, device=self.device)
